...
def meroshare_login(dp,username,password):
...
def open_ipo_lister():
    ...
    IPOlist = web_driver.driver.find_elements(By.CLASS_NAME,"company-name")  # Find the elements by XPATH and find its length
...
def apply_ipo(kitta,crn,txn_pin):
    ...
    if len(bank_list) > 2:  # check if there are multiple bank accounts
        # print empty line
        print('\n')
    ...
    if "successfully" in msg:  # if message contains "Success" then print msg in green else print in red
